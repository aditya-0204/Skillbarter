...
import { 
  ...
  onAcceptedSwapsSnapshot, // Use the new real-time listener
  acceptSwapRequest 
...
export default function MySwaps() {
  ...
  // Fetch confirmed sessions in real-time
  useEffect(() => {
    if (!currentUser) {
  ...
  const handleConfirmSwap = async (selectedSkillFromRequester) => {
    ...
    try {
      // This updates the database. The listeners will automatically update the UI.
      await acceptSwapRequest(currentRequest.id, skillToOffer);
    } catch (err) {
      console.error('Error confirming swap:', err);
    }
    // No need to manually update state anymore
